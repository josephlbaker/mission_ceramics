...
export default class Nav extends Component {
  ...
  render() {
    let headerClassName = this.props.onCart ? 'toolbar scrolled' : this.state.isTop ? 'toolbar' : 'toolbar scrolled';
    let navClassName = this.props.onCart ? 'cart-toolbar__logo' : this.state.isTop ? 'toolbar__logo' : 'toolbar__logo scrolled';
    let cartIconClassName = this.props.itemsInCart ? 'visible' : 'hidden';
    return (
      <header className={headerClassName}>
        <nav className="toolbar__navigation">
          <NavLink exact to="/"><div className={navClassName} onClick={this.props.toggleHome}>Mission Ceramics</div></NavLink>
          <div className="spacer" />
          <div className="toolbar_navigation-items">
            <ul>
              <NavLink to="/cart" activeClassName="active" onClick={this.props.toggleCart} className={cartIconClassName}>
