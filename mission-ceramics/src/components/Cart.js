...
export default class Cart extends Component {
  ...
  createOrder = (line_items) => {
    fetch('http://167.172.127.78/api/orders', {
      method: 'POST',
      headers: {
  ...
  createCheckout = (order) => {
    fetch('http://167.172.127.78/api/checkout', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      ...
      body: JSON.stringify({
        ...
        redirect_url: "http://167.172.127.78/"
      })
    })
      .then(response => {
        response.json().then(data => ({
    ...
    if (this.props.cart.length > 0) {
      ...
      return (
        <div className="cart-empty">
          ...
          <h3>Click <a href="http://167.172.127.78/">here</a> to continue shopping.</h3>
        </div>
      )
    }
  }
