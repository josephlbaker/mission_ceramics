...
export default class Cart extends Component {
  ...
  createOrder = (line_items) => {
    fetch('http://test-app.missionceramics.com:8000/orders', {
      method: 'POST',
      headers: {
  ...
  createCheckout = (order) => {
    fetch('http://test-app.missionceramics.com:8000/checkout', {
      method: 'POST',
      headers: {
        'Accept': 'application/json',
        'Content-Type': 'application/json'
      ...
      body: JSON.stringify({
        ...
        redirect_url: "http://test-app.missionceramics.com"
      })
    })
      .then(response => {
        response.json().then(data => ({
    ...
    if (this.props.cart.length > 0) {
      ...
      return (
        <div className="cart-empty">
          ...
          <h3>Click <a href="http://test-app.missionceramics.com">here</a> to continue shopping.</h3>
        </div>
      )
    }
  }
